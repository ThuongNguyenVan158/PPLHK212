...
from main.d96.utils.Visitor import BaseVisitor
...
class GetEnv(BaseVisitor, Utils):
    ...
    def visitClassDecl(self, ast: ClassDecl, o: object):
        ...
        #     self.visit(x, o[classname])
        classname = ast.classname.name
        if o.get(ast.classname.name) is not None:
            raise Redeclared(Class(), classname)
        o[classname] = {}
        [self.visit(x, o[classname]) for x in ast.memlist]
    ...
    def visitAttributeDecl(self, ast: AttributeDecl, o: object):
        ...
        if type(ast.kind) is Instance:
            self.visit(ast.decl, ('instance', o))
        else: self.visit(ast.decl,('static', o))
    ...
    def visitMethodDecl(self, ast: MethodDecl, o: object):
        ...
        # if type(ast.kind) is Static:
        #     kind = 'static'
        # if o.get(ast.name.name) is not None:
        #     raise Redeclared(Method(), ast.name.name)
        # params = ast.param
        # if ast.returnType is not None:
        #     param = list(map(lambda x: self.visit(x, (None, {})), params))
        #     o[ast.name.name] = ('method', self.visit(ast.returnType, o), kind, param)
        # else:
        #     param = list(map(lambda x: self.visit(x, (None, {})), params))
        #     o[ast.name.name] = ('method', None, kind, param)
        if type(ast.kind) is Instance:
            kind = 'instance'
        else: kind = 'static'
        if o.get(ast.name.name) is not None:
            raise Redeclared(Method(), ast.name.name)
        param = list(map(lambda x: self.visit(x,(None,{})), ast.param))
        o[ast.name.name] = ('method', None, kind, param)
...
class StaticChecker(BaseVisitor,Utils):
    ...
    def visitProgram(self, ast, c):
        ...
        print(env)
        a= [self.visit(x, env) for x in ast.decl]
    ...
    def visitMethodDecl(self, ast: MethodDecl, o):
        ...
        env['current'] = o['current']
        env['parent'] = o['parent']
        [self.visit(x, (Parameter(), env)) for x in ast.param]
