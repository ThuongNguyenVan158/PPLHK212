...
class GetEnv(BaseVisitor, Utils):
    ...
    def visitVarDecl(self, ast: VarDecl, o: object):
        ...
        # name = ast.variable.name
        # if o[1].get(name) is not None:
        #     raise Redeclared(Attribute(), name)
        # o[1][name] = ('mutable', self.visit(ast.varType, o[1]), o[0])
        # return('mutable', self.visit(ast.varType, o[1]), kind)
    ...
    def visitMethodDecl(self, ast: MethodDecl, o: object):
        ...
        if ast.returnType is not None:
            ...
            o[ast.name.name] = ('method', self.visit(
                ast.returnType, o), kind, param)
        else:
            param = list(map(lambda x: self.visit(x, (None, {})), params))
