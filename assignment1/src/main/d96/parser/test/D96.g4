...
/****************************************************************************/
att_declare: (VAL|VAR) (att_names Extended data_type | att_init_value) SM;
...
func_declare: idname LP paramlist RP blockstatment;
paramlist: params | ;
params: param SM params | param;
param: idnamelist Extended data_type;
idnamelist: idname CM idnamelist | idname;
...
index_array_literal: ARRAY LP primitive_literal (CM primitive_literal)* RP; 
...
scalarvar: ID; // check is need $
indexed_exp: exp index_operator | (SELF '.')? ID (index_operator)* | SELF? ID ('.' ID)+;
...
return_stm: RETURN (exp | DOLLARID) SM;
...
exp7: exp7 DOT exp8 | exp8;
exp8: exp9 SC static_operand | exp9;
exp9: NEW exp9| exp10;
exp10: literal| TRUE | FALSE | ID | SELF | func_call | LP exp RP; // check is have self
/****************************************************************************/
/*									Member access							*/
/****************************************************************************/
...
method_invocate: object_exp DOT ID LP explists RP;

object_exp: ID| NEW func_call | SELF| SELF? ID ('.' ID)* ('.' func_call)? object_exp| object_exp1 ;
object_exp1: LP object_exp RP;
explists: explist | ;
explist: exp CM explist | exp;

static_method_invocate: ID SC DOLLARID LP explists RP;
...
/*								3.7 Literals								*/
/****************************************************************************/
INTERGER_GT_ZERO: (DECIMAL_INTEGER_GT_ZERO| OCT_INTEGER_GT_ZERO| HEX_INTEGER_GT_ZERO| BIN_INTEGER_GT_ZERO) ;
