...
scalarvar: ID | exp | ID SC DOLLARID ((index_operator)* | ('.' ID)+ ) | (SELF '.')? ID (index_operator)* | SELF? ID ('.' ID)+ | ID SC DOLLARID; // check is need $
...
object_exp: ID | NEW func_call | SELF| SELF? ID ('.' ID)* | exp;
explists: explist | ;
